...
### 2026-05-13 核心功能修复与交互稳定补丁

状态：已完成，等待用户验收。

实现范围：
- 修复任务创建链路的可见性与选中状态：`createTask` 现在返回新任务并在刷新后选中新任务；任务页增加日期筛选，默认今日视图包含今日任务、无日期未完成任务，以及重要/紧急的逾期未完成任务。
- 任务表单继续使用原生 `type="date"` 与 `type="time"`，默认本地今天和当前分钟，提交时合并为 `deadline` 并同步写入 `planned_date`；详情页可回显并修改日期时间。
- 计时器三模式补齐交互：正计时 00:00 起步，番茄钟 15/25/30/45/60 分钟预设加自定义分钟，倒计时支持小时和分钟并记住上次时长；保留 Rust/Tauri 后端计时与 `timer_tick`。
- AI 对话列表改为独立细滚动条，输入区固定在底部；AI 页面扩展为工作区骨架，包含对话区、结果预览、文件上传入口和学习/考试/资料规划占位入口；工作台 AI 卡片增加“打开 AI 工作区”按钮。
- CalendarView 月视图扩展为 42 格，头部增加上个月/下个月箭头和居中年月，当前日期高亮，滚动条使用玻璃风细滚动条。
- 四象限标题字号和数量展示增强，日期筛选条支持今天、明天、本周、全部和自定义日期。
- 复用现有统计页真实数据图表与水波 TimerOrb，未改数据库迁移或 Rust 计时核心。

验收结果：
- `npm run build` 通过。
- 构建仍提示现有 Vite chunk size 与 ineffective dynamic import warning，不影响本轮功能修复。

TODO：
- 后续可做真实浏览器/Tauri 手工验收：创建任务持久化、Tauri `create_task` 入库、倒计时结束关联任务流程、AI SSE 长消息滚动、日历拖拽改期。

## Future Backlog / Roadmap

说明：本章节仅规划后续开发，不扩大当前 2026-05-13 Hotfix 范围。当前 Hotfix 只保留任务创建、日期/时间输入、计时器三模式、AI 消息滚动、日历月份切换、四象限基础筛选、统计页基础增强和计时器水波动画基础版等必须修复项。文件上传、学习资料规划、荣誉墙、24 小时时间环、完整 AI 工作区和外部日历同步均进入后续 Sprint。

### Roadmap 合并关系

- 合并到已有 Sprint 11：智能排程与日历联动增强、完整月/周/日历视图、任务拖拽改期、冲突/过载检测、计划 vs 实际、Google Calendar / Outlook 适配层预留。
- 合并到已有 Sprint 12：计时器高级模式、情境感知任务推荐、结束后关联建议、历史专注记录筛选、水波动画物理液面升级。
- 合并到已有 Sprint 13：Achievements 荣誉墙、徽章锁定/解锁、连续专注、番茄钟成就、完成任务成就、虚拟花园联动。
- 合并到已有 Sprint 14：统计页 2.0、7 天趋势、任务完成趋势、四象限分布、完成/未完成/逾期分布、专注时段分析、计划 vs 实际、24 小时时间环。
- 合并到已有 Sprint 15：AI 周报、AI 生成周总结、图表解释文案、数据不足空状态。
- 合并到已有 Sprint 16：第二代视觉系统收口、玻璃打光层级、hover 动效、统一滚动条、light/dark 可读性、关键视口验收。
- 新增 Future Sprint 17：核心交互稳定与任务页增强，承接四象限增强、历史任务顺延/待整理、日期筛选、拖拽与批量整理。
- 新增 Future Sprint 18：AI 工作区 2.0，承接独立 AI 页面、结构化预览、计划生成、一键转任务/日程和文件上传入口占位。
- 新增 Future Sprint 19：文件上传与学习资料规划，承接 PDF / Word / Markdown / 图片资料列表、元数据、后续 OCR/文档解析适配。
- 仅放入 Backlog 暂不开发：真实 OCR、完整文档内容解析、第三方日历双向同步、跨设备云同步、团队协作、插件系统和移动端。
...
### Sprint 20：Workbench 交互联动增强

目标：
- 让首页 Quick Stats 从静态数字升级为可点击的导航和详情入口，同时不破坏 Workbench 首页布局。

Implementation：
- 点击“今日专注”进入计时记录 / 统计详情；如果详情页尚未完成，打开同主题悬浮详情窗。
- 点击“完成率”进入统计页对应模块或完成率详情窗。
- 点击“未完成”进入任务页并筛选未完成任务。
- 点击“今日计时”进入今日计时记录或详情窗。
- 悬浮窗保持玻璃拟态主题，支持关闭、Esc、点击遮罩关闭。
- 点击行为不得改变 Workbench 卡片高度，不得让 Quick Stats 覆盖 Achievements。
- 为跳转行为增加明确的 view/filter 状态传递方案。

Acceptance：
- 四个 Quick Stats 均可点击，并进入正确页面或打开正确详情窗。
- 从详情窗返回不会丢失 Workbench 当前计时、日期选择和 AI 输入状态。
- 窄屏下悬浮窗不溢出视口，内容可滚动。

依赖：
- 当前 Workbench Quick Stats 真实数据联动。
- Sprint 14 统计页详情模块可作为最终落点；未完成前先用详情窗。

状态：未开始。

TODO：
- 决定跨页面筛选状态放在 Zustand view params，还是组件局部 query state。
- 设计详情窗最小字段，避免提前实现完整统计页 2.0。

### Sprint 21：测试、打包与发布验收

目标：
- 在 Future Roadmap 主要功能完成后，进行系统性测试、性能检查、打包和发布验收。
...
- 打包前确认 `docs/PRD.md`、`plan.md` 和迁移文件已同步。
...
- 发布前更新 README 或发布说明，但不在本 Roadmap 文档任务中执行。
"## 2026-05-13 工作台 AI 输入框样式对齐微调"  
"- 缩小了 AI 面板输入框 form 的 mb (margin-bottom) 间距，让输入框适度靠向卡片底部，不过度留白。" 
"- [x] 调整 AI 助手面板样式：输入框贴底并增大高度、加宽语音按钮、迎宾模块图标上置对齐。" 
