...
状态：进行中（Sprint 17A 已完成，2026-05-14；Sprint 17 剩余项待后续确认）
...
状态：部分完成（2026-05-14 已完成 Sprint 17A：任务页增强，无数据库迁移版）
...
剩余 TODO：
- 如需“待整理”自动整理入口或独立分组，继续基于 tags 方案细化交互，不在无迁移版中新增字段。
- 继续补充 Rust 层或集成测试，覆盖 `urgency` / `importance` 更新后的 `quadrant` 计算与批量整理持久化。
- 后续可补充浏览器视口验收，覆盖任务页在窄屏下的批量工具栏可读性。

下一 Sprint 建议：
- 先验收 Sprint 17A 的任务页交互；下一轮仍建议继续 Sprint 17 的稳定性补测和小修，不直接跳到 Sprint 11/12/18/19 等被禁止范围。
